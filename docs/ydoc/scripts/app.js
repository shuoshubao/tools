var $panel = document.getElementById("js-panel"),
  ...
  $summaryItems = Array.prototype.slice.call(
    document.querySelectorAll("#js-menu .href")
  ),
...
function addEvents() {
  ...
  if (
    sessionStorage.prevPrevPathname !== window.location.pathname &&
    sessionStorage.prevPathname !== window.location.pathname
  ) {
    sessionStorage.setItem("contentScrollTop", "0");
  } else if (sessionStorage.prevPrevPathname === window.location.pathname) {
    sessionStorage.setItem(
      "contentScrollTop",
      sessionStorage.prevPrevContentScrollTop
    );
  }
}
