$(function () {
  ...
  var searchMaxNum = 8;
  var releativePath =
    document.getElementById("releativePath").getAttribute("content") || "";
  ...
  function searchBook(pages) {
    ...
    for (var i = 0, l = pages.length; i < l; i++) {
      ...
      if (
        page.content &&
        page.content.toLowerCase().indexOf(searchText.toLowerCase()) !== -1
      ) {
        num++;
        searchPage = {
          title: page.title,
      ...
      if (page.children && Array.isArray(page.children)) {
        for (var j = 0, len = page.children.length; j < len; j++) {
          ...
          if (
            child.content &&
            child.content.toLowerCase().indexOf(searchText.toLowerCase()) !== -1
          ) {
